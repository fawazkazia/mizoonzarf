...
import { Pencil, Trash2, Minus, Plus, X, ChevronRight, Gift, ShieldCheck, BadgeCheck, Truck } from "lucide-react";
      ...
      <form onSubmit={handleSubmit} className="grid gap-4 lg:grid-cols-[1fr_360px] lg:items-start lg:gap-6">
        <div className="flex min-w-0 flex-col gap-4">
          <section className="border border-line p-4 sm:p-5">
            <h2 className="mb-3 text-xs font-medium uppercase tracking-[0.1em] sm:text-sm">Contact Information</h2>
            <input
              type="email"
              required
              value={email}
          ...
          <section className="border border-line p-4 sm:p-5">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-xs font-medium uppercase tracking-[0.1em] sm:text-sm">Delivery Address</h2>
              {!addressExpanded && (
                <button type="button" onClick={() => setAddressExpanded(true)} className="text-xs font-medium uppercase tracking-[0.08em] underline underline-offset-2">
                  {addressSummary ? "Change" : "Add"}
                </button>
              )}
            </div>
            ...
            {!addressExpanded && addressSummary && (
              <div className="text-sm">
                <p className="font-medium">{addressSummary.name}</p>
                <p className="text-ink-soft">
                  {addressSummary.line1}, {addressSummary.city}, {addressSummary.country}
                </p>
                <p className="text-ink-soft">{addressSummary.phone}</p>
              </div>
            )}
            ...
            {!addressExpanded && !addressSummary && (
              <button
                ...
                className="flex w-full items-center justify-between border border-dashed border-line-strong px-4 py-3 text-left text-sm text-ink-soft"
              >
                ...
                {savedAddresses.length > 0 && (
                  <div className="mb-4 flex flex-col gap-2">
                    {savedAddresses.map((a) =>
                      editingSavedId === a.id ? (
                        <div key={a.id} className="border border-ink p-4">
                        ...
                        <div key={a.id} className={`border p-3 text-sm ${selectedAddressId === a.id ? "border-ink" : "border-line"}`}>
          ...
          <section className="border border-line p-4 sm:p-5">
            <div className="mb-2.5 flex items-baseline justify-between">
              <h2 className="text-xs font-medium uppercase tracking-[0.1em] sm:text-sm">Shipment 1 of 1</h2>
            ...
            <ul className="flex flex-col divide-y divide-line">
              {cart.lines.map((line) => (
                <li key={line.id} className="flex gap-3 py-3 first:pt-0 last:pb-0">
                  <div className="relative h-20 w-16 shrink-0 bg-paper-dim">
          ...
          <section className="border border-line p-4 sm:p-5">
            <h2 className="mb-3 text-xs font-medium uppercase tracking-[0.1em] sm:text-sm">Delivery Instructions (Optional)</h2>
            <textarea
              value={notes}
          ...
          <section className="border border-line p-4 sm:p-5">
            <h2 className="mb-3 text-xs font-medium uppercase tracking-[0.1em] sm:text-sm">Delivery Method</h2>
            <div className="flex flex-col gap-2">
              ...
              <label className={`flex cursor-pointer justify-between border p-3 text-sm ${deliveryMethod === "express" ? "border-ink" : "border-line"}`}>
                ...
                <span>{formatINR(settings.shipping.expressFee)}</span>
              </label>
            </div>
          </section>
        </div>
        ...
        <div className="flex min-w-0 flex-col gap-3.5 lg:sticky lg:top-24">
          {!isSignedIn && (
            <div className="border border-gold-soft bg-paper-dim p-3.5 text-sm">
          ...
          <div className="border border-line p-4 sm:p-5">
            <h2 className="mb-3 font-display text-base sm:text-lg">Order Details</h2>

            {isSignedIn && pointsEstimate > 0 && (
              <div className="mb-3 flex items-center gap-2 bg-paper-dim px-3 py-2 text-xs">
            ...
            <ul className="mb-3 flex flex-col gap-2.5">
              {cart.lines.map((line) => (
                <li key={line.id} className="flex gap-3">
                  <div className="relative h-14 w-12 shrink-0 bg-paper-dim">
                    <Img src={line.image} alt={line.productName} seedFallback={line.variantId} />
            ...
            <div className="flex flex-col gap-2 border-t border-line pt-3 text-sm">
              ...
              <div className="mt-1.5 flex items-center justify-between bg-paper-dim px-3 py-2.5 text-base font-semibold">
                <span>Total</span>
            ...
            <Button
              ...
              size="lg"
              disabled={loading}
              className="mt-4 w-full justify-between rounded-full py-3.5 normal-case tracking-normal"
            >
              <span>{formatINR(cart.total)}</span>
          ...
          <div className="border border-line p-4 sm:p-5">
            <div className="flex items-center justify-between">
              <h2 className="text-xs font-medium uppercase tracking-[0.1em] sm:text-sm">Promo Code</h2>
            ...
            {cart.couponCode && (
              <div className="mt-3 flex items-center justify-between bg-paper-dim px-3 py-2 text-sm">
                <span>
                ...
                {couponOffers.length > 0 && (
                  <ul className="flex flex-col gap-2">
                    {couponOffers.map((offer) => (
                      <li key={offer.code} className="flex items-center justify-between border border-line px-3 py-2 text-sm">
                        <span>
          ...
          {offers.length > 0 && (
            <div className="border border-line p-4 sm:p-5">
              <h2 className="mb-3 text-xs font-medium uppercase tracking-[0.1em] sm:text-sm">Offers For You</h2>
          ...
          <div className="grid grid-cols-3 gap-2 border-t border-line pt-3 text-center text-[11px] text-ink-soft">
